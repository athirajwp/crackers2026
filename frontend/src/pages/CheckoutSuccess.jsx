...
export default function CheckoutSuccess() {
  ...
  const initialData = location.state && location.state.order
    ? { order: location.state.order, whatsappUrl: location.state.whatsappUrl }
    : null;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-sm font-semibold text-slate-500">Loading enquiry details...</p>
      </div>
    );
  }
  ...
  if (!data || !data.order) {
    return (
      <div className="text-center py-20">
        <h2 className="text-2xl font-black text-rose-600">Enquiry Not Found!</h2>
        <p className="text-xs text-slate-500 font-semibold mt-2">
          We couldn't retrieve the enquiry details for reference: {orderNumber}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl text-slate-800">
      ...
      <div className="text-center space-y-4 mb-10 select-none">
        ...
        <h2 className="text-3xl font-extrabold tracking-tight text-slate-950">Enquiry Submitted Successfully!</h2>
        <p className="text-xs text-slate-500 max-w-lg mx-auto leading-relaxed font-semibold">
          Your enquiry has been received. Please click the WhatsApp button below to confirm your requirement details and coordinate offline fulfillment options!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
        ...
        <div className="md:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div className="flex justify-between items-center border-b border-slate-200 pb-4">
            <div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Enquiry Details</h3>
            ...
            >
              <i className="fa-solid fa-print text-crimson-600"></i> Print Enquiry
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs text-slate-655 font-semibold">
            ...
            <div className="space-y-2.5">
              <div className="text-slate-400 uppercase tracking-wider text-[10px] font-bold">Enquiry Information</div>
          ...
          {/* Subtotal & Savings Panel (Excludes Net Amount Payable) */}
          ...
          {/* Short Statutory Legal Notice */}
          <div className="bg-amber-50/90 border border-amber-200/90 rounded-xl p-3.5 text-xs text-amber-950 font-medium leading-relaxed flex items-start gap-2.5">
            <i className="fa-solid fa-triangle-exclamation text-amber-600 text-sm flex-shrink-0 mt-0.5"></i>
            <div>
              <strong className="font-bold">Statutory Legal Notice:</strong> This website is for product information and enquiry purposes only and does not accept online firecracker orders or online payments. All purchases are fulfilled through authorised and licensed channels in accordance with applicable laws and local regulations.
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4 text-center">
            ...
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-widest font-black">Enquiry Follow-up</h4>
            <p className="text-[10px] text-slate-500 leading-relaxed font-semibold">
              Please share your enquiry details with us on WhatsApp to discuss your requirements and offline options!
            </p>
            <a
              href={whatsappUrl}
              target="_blank"
